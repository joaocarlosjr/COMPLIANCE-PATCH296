...
is
   ...
   vn_nro_nf_antigo     Nota_Fiscal.Nro_Nf%type     := null; --#73872
   vn_nro_nf_novo       Nota_Fiscal.Nro_Nf%type     := null;
   vn_dm_tipo_integr    Empresa.Dm_Tipo_Integr%type := null;
   vv_vlr_parametro     param_geral_sistema.vlr_param%type := null;
   --
begin
   --
   vn_fase := 1;
   ...
   -- Recupera valor do parametro e dm_tipo_integr para a empresa. #73872
   begin
      select   prm.vlr_param
        ...
        from csf_own.param_geral_sistema prm,
             csf_own.empresa emp
       where emp.id                      = prm.empresa_id
         and prm.empresa_id              = est_row_Nota_Fiscal.empresa_id
         ...
         and prm.grupo_id                = (select id
                                             ...
                                             where UPPER(cod_grupo) = UPPER('NFSE')
                                               and modulo_id = (select id
                                                                  from csf_own.modulo_sistema
                                                                 where UPPER(cod_modulo) = UPPER('EMISSAO_DOC')))
         and UPPER(prm.param_name)       = UPPER('CONTROLA_NRO_RPS');
   exception
     when others then
   ...
   -- Verifica se o parametro esta habilitado para a empresa e dm_tipo_integr diferente de 3 e 4, para validacao o parametro precisa ser '1'. #73872
     if (nvl(vv_vlr_parametro, 0) = '1' and nvl(vn_dm_tipo_integr, 0) <> 3 and nvl(vn_dm_tipo_integr, 0) <> 4) then
        --        
        begin
          select prox_nro_nf
            into vn_nro_nf_novo
            from EMPRESA_CTRL_NRO_NF
            where empresa_id    = est_row_Nota_Fiscal.empresa_id  
              and serie         = est_row_Nota_Fiscal.serie
               and modfiscal_id = est_row_Nota_Fiscal.modfiscal_id;
        exception
          when others then
            vn_nro_nf_novo := null;
        end;
        --
        if (vn_nro_nf_novo is null) then
          --
          gv_mensagem_log := 'Não foi encontrado parametro de controle de numeração para a ' ||
          'série ' || est_row_Nota_Fiscal.serie || ', criar tal parametro ou inativar o parametro geral CONTROLA_NRO_RPS.';
          --
          vn_loggenericonf_id := null;
          --
          pkb_log_generico_nf ( sn_loggenericonf_id  => vn_loggenericonf_id
          ...
          --
          vn_nro_nf_antigo           := est_row_Nota_Fiscal.nro_nf;
          est_row_Nota_Fiscal.nro_nf := vn_nro_nf_novo;          
        end if;
   ...
   if     nvl(est_row_Nota_Fiscal.empresa_id, 0)       > 0
      ...
      --Salva número novo na tabela EMPRESA_CTRL_NRO_NF e grava Log referente a troca de número #73872
      if (vn_nro_nf_novo is not null) then
          --Alteracao de numeracao de controle para nota de serviço
          begin
            update EMPRESA_CTRL_NRO_NF
               set prox_nro_nf = vn_nro_nf_novo + 1
             where empresa_id    = est_row_Nota_Fiscal.empresa_id   
               and serie         = est_row_Nota_Fiscal.serie
               and modfiscal_id  = est_row_Nota_Fiscal.modfiscal_id;
               --
          ...
          exception
            when others then
              gv_mensagem_log := 'Erro ao alterar Controle de Numeração (EMPRESA_CTRL_NRO_NF) Fase: ' || vn_fase || '.';
              --
              vn_loggenericonf_id := null;
              --
              pkb_log_generico_nf ( sn_loggenericonf_id  => vn_loggenericonf_id
          ...
          --
          gv_mensagem_log := 'Alteração de numero de RPS de ' || vn_nro_nf_antigo || ' para ' || vn_nro_nf_novo || ' conforme ' ||
          'determinado em parametro da série ' || est_row_Nota_Fiscal.serie || '.';
          --
          vn_loggenericonf_id := null;
          --
          pkb_log_generico_nf ( sn_loggenericonf_id  => vn_loggenericonf_id
